...
def _block(r):
    ...
    return (
        ...
        "| Metric | Deterministic + judge + runtime authorization |\n|---|---|\n"
        f"| **Unauthorized-action execution rate (harm ran)** | **{p(r.get('unauthorized_action_execution_rate'))}** |\n"
        f"| Unsafe-action approval rate (pre-filter approved → authz denied) | {p(r.get('unsafe_action_approval_rate'))} |\n"
        f"| Runtime-authorization saves | {r.get('runtime_authorization_saves','?')} |\n"
